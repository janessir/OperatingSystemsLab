...
void 
Timer::myfunction() //TimeHandler and TimeOfNextInterrupt variables are local to this class, and hence myfunction() is needed to help invoke scheduler on them (since it's in Timer class and has access to those vars); which can be called outside the class, ie in thread.cc
{
...
Timer::Timer(VoidFunctionPtr timerHandler, _int callArg, bool doRandom)
{
    ...
    interrupt->Schedule(TimerHandler, (_int) this, TimeOfNextInterrupt(), 
		TimerInt);  //TimerOfNextInterrupt() shld return 40
}
